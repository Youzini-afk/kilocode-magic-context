 ...
 *      whether they're wrapping `Part` from `@kilocode/sdk` or
 ...
 * interface — it never imports from `@kilocode/sdk` or
 * `@mariozechner/pi-ai`.
 */
export interface Transcript {
     ...
     * - Logging (`magic-context[kilo]` vs `magic-context[pi]`).
     * - Per-harness behaviors gated at adapter level (e.g. Kilo-only
     *   compaction marker injection).
     * - Test assertions confirming the right adapter ran.
     */
    readonly harness: "kilo" | "opencode" | "pi";
