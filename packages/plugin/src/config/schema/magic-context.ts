...
/** Valid reasoning levels for Kilo-compatible subagents.
 *  Off disables reasoning; minimal/low/medium/high/xhigh/max pass through
 *  to providers that support expanded reasoning controls. */
...
export const DreamerConfigSchema = AgentOverrideConfigSchema.merge(
    z.object({
        ...
        /** Optional reasoning level for dreamer subagent tasks. See HistorianConfigSchema. */
...
export const SidekickConfigSchema = AgentOverrideConfigSchema.extend({
    ...
    /** Optional reasoning level for sidekick subagent. See HistorianConfigSchema. */
    thinking_level: PiThinkingLevelSchema,
}).optional();
...
export const HistorianConfigSchema = AgentOverrideConfigSchema.extend({
    ...
    /** Optional reasoning level passed through to subagent
     *  invocations. Required when using reasoning models (e.g. github-copilot/gpt-5.4)
     *  because the default reasoning resolution can pick a value the provider
     *  rejects. You can also set Kilo's agent `variant` override directly.
...
export interface MagicContextConfig {
    ...
    /** Per-model context limit overrides. Highest-priority source for Kilo Magic Context budget math. */
    model_context_limits?: { default?: number; [modelKey: string]: number | undefined };
...
export const MagicContextConfigSchema = z
    .object({
        ...
        execute_threshold_tokens: z
            ...
            .optional(),
        /** Per-model context limit overrides. Accepts provider/model, bare model, or `default`.
         *  These values override Kilo's model catalog/API cache for Magic Context budget math. */
        model_context_limits: z
            .object({
                default: z.number().min(5_000).max(5_000_000).optional(),
            })
            .catchall(z.number().min(5_000).max(5_000_000))
            .optional(),
