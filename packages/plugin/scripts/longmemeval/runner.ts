...
import { createKiloClient, type AssistantMessage, type Part } from "@kilocode/sdk";
...
type OpencodeClient = ReturnType<typeof createKiloClient>;
...
async function createSession(
    ...
    const data = (response as ResponseWithData<{ id?: unknown }>).data;
...
async function fetchMessages(
    ...
    return Array.isArray(data) ? (data as SessionMessageShape[]) : [];
}
...
async function sendPromptAndCapture(
    ...
    const directData = (response as ResponseWithData<SessionMessageShape>).data;
