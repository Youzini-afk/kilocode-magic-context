...
import { getKiloDbPath, getKiloStorageDir, getMagicContextStorageDir } from "../../src/shared/data-path"
...
export function resolveOpenCodeDatabasePath(): string {
	const explicit = process.env.KILO_DB
	if (explicit && existsSync(explicit)) {
		return explicit
	}

	const defaultDb = getKiloDbPath()
	if (existsSync(defaultDb)) {
		return defaultDb
	}

	const channelDbCandidates = listDatabaseFiles(getKiloStorageDir(), "kilo")
	if (channelDbCandidates.length > 0) {
		return channelDbCandidates[0]
	}

	const storageDbCandidates = listDatabaseFiles(getKiloStorageDir(), "")
	if (storageDbCandidates.length > 0) {
		return storageDbCandidates[0]
	}

	throw new Error(
		`Unable to locate Kilo DB. Checked ${defaultDb} and storage DBs in ${getKiloStorageDir()}`,
	)
}
...
export async function createDumpFilePath(sessionId: string, timestamp: string): Promise<string> {
	const dumpDir = join(process.cwd(), ".kilo", "dumps")
